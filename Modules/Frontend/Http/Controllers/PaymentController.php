        ...
        if ($promotion_id) {
            ...
            if ($promotion) {
                if ($promotion->discount_type == 'fixed') {
                    ...
                    $couponDiscount = ($plan->total_price * $promotion->discount) / 100;
                }
    
                // Ensure the discount does not exceed the plan price
                $couponDiscount = min($couponDiscount, $plan->price);
    
                // Apply the discount to the price
                $discountedPrice = $plan->total_price - $couponDiscount;
            }
        }
